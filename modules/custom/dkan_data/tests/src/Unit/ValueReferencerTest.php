...
class ValueReferencerTest extends DkanTestBase {
  ...
  public function testConstruct() {
    ...
    $mockConfigFactoryInterface = $this->createMock(ConfigFactoryInterface::class);
    $mockQueueFactory      = $this->createMock(QueueFactory::class);

    // Assert.
    $mock->__construct($mockEntityTypeManager, $mockUuidInterface, $mockConfigFactoryInterface, $mockQueueFactory);
    ...
    $this->assertSame($mockConfigFactoryInterface, $this->readAttribute($mock, 'configService'));
   ...
   *   Expected result.
   */
  public function testCheckExistingReference(string $property_id, $data, array $nodes, $expected) {
    // Setup.
    $mock = $this->getMockBuilder(ValueReferencer::class)
      ->setMethods(NULL)
      ->disableOriginalConstructor()
   ...
   * @param $expected
   */
  public function testDereferenceProperty(string $property_id, $data, $derefMultiple, $derefSingle, $expected) {
    // Setup.
    $mock = $this->getMockBuilder(ValueReferencer::class)
      ->disableOriginalConstructor()
    ...
    // Assert.
    $actual = $this->invokeProtectedMethod($mock, 'dereferenceProperty', $property_id, $data);
    $this->assertEquals($expected, $actual);
  }
